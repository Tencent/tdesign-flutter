...
### 数据源选型：TPickerColumns vs TPickerLinked

| | TPickerColumns | TPickerLinked |
| --- | --- | --- |
| 语义 | 多列**独立**，互不影响 | **联动树**，改上游刷新下游 |
| 数据结构 | `List<List<TPickerOption>>` | `Map<TPickerOption, dynamic>` |
| 典型场景 | 年月日、双列自选、列底分页 | 省市区（整树在内存） |
| 改第 1 列 | 只影响第 1 列 | 下游列整列换新，默认首项 |
| 远程分页 | ✅ 配合 `onColumnScrollEnd` | ❌ 请改用 Columns + 业务 Scope |

远程逐级拉取见 example `LinkedLazyPickerScope`。

### fromRaw 是什么

`fromRaw` = **松散数据入口**：把接口 JSON、字面量 `String`/`Map` 等自动归一化为 `TPickerOption`，无需手写逐条转换。

| 归一化规则 | 行为 |
| --- | --- |
| 已是 `TPickerOption` | 透传 |
| `Map` | 按 `TPickerKeys` 读 label/value/disabled |
| 其他（如 `String`） | label 为 `toString()`，value 为原值 |
| 已是目标类型 | 短路返回，不深拷贝 |
...
已全是 `TPickerOption` 时用直接构造，不必 `fromRaw`：

```dart
TPickerColumns([[TPickerOption(label: '北京', value: 'BJ')]])
TPickerLinked({TPickerOption(label: '广东', value: 'GD'): {...}})
```
...
| 场景 | 推荐做法 | 避免 |
|------|----------|------|
| 列尾分页 append | 原地 `addAll` 或 immutable 追加 | 每帧回写 `initialValue` |
| 联动换子列 | 仅替换后续某一列 | 双列全量替换导致主列滚动位置丢失 |
| 实时选中 | 由 `onChange` / 业务 draft 维护 | 用 `initialValue` 当滚动中的选中源 |

### 回调与初始值

- `onChange`：滚动实时 draft；多列独立惯性可能多次，联动同帧合并一次；`col` 为触发列
- `onColumnScrollEnd`：滚停时触发，适合列底分页（判断 `value.indexes[col]`）
- `initialValue`：仅首次构建生效；运行期选中用 `onChange`；重置用 `Key` 或改 `items`
...
| `colIndex` | 列索引 |
| `index` | 行索引 |
| `itemDistanceCalculator` | 距离样式计算器（可复用 4 档默认颜色/字号） |
| `distance` | 离中心距离（0 表示选中项） |
| 返回值 | `Widget?`；`null` 时用默认 `TText`，disabled 项不走此 builder |
