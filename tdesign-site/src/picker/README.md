...
| items | `TPickerItems` | 必填 | 数据源。密封类 `TPickerItems` 编译期强制二选一：<br>- **`TPickerColumns`**：多列独立选择<br>- **`TPickerLinked`**：联动选择<br><br>自由结构数据通过 `.fromRaw()` 工厂构造并自动归一化。实例值相对上一帧不相等时触发整组重初始化；内容相等的新实例不重建。 |
| initialValue | `List<dynamic>?` | - | 初始选中值列表（按 `value` 匹配各列）。<br>- **语义**：initState-only —— 仅首次构建生效，后续传入被忽略<br>- **机制**：`FixedExtentScrollController` 拥有滚动位置所有权，频繁回灌会触发 `dispose+reinit`，破坏惯性滚动<br>- **典型症状**：滚轮"每次只能滚 1 项"<br>- **正确做法**：选中态用 `onChange` 维护 draft；"重置"时用 `Key` 强制重建；数据源变更时改 `items` |
| onChange | `void Function(int col, TPickerValue value)?` | - | 值改变回调（滚动时实时触发）。<br>- **`col`**：触发的列索引（0-based，从左到右）<br>- **`value`**：当前各列选中快照<br><br>触发时机：用户滚动 / disabled 修正完成。此回调代表实时变化，不代表用户已确认选择；弹窗场景请配合 `TPopup` 头部确认按钮提交 draft。 |
| onColumnScrollEnd | `void Function(int col, TPickerValue value)?` | - | 指定列滚动结束回调（惯性停止或手指抬起后）。<br>- **参数**：同 `onChange`<br>- **用途**：判断 `value.indexes[col]` 是否接近列底并触发分页，避免在 `onChange` 里做高频加载判定 |
| height | `double` | `200` | 滚轮视窗高度（像素）。 |
| itemCount | `int` | `5` | 每屏显示 item 数。 |
| disabled | `bool` | `false` | 是否禁用整个选择器（禁止滚动和操作）。禁用时同时屏蔽无障碍手势。 |
