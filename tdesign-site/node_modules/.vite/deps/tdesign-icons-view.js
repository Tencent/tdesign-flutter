...
var global_default = typeof window === "object" ? window : polyfill(globalThis);
...
function dispatch(target) {
  if (callbacks.has(target)) {
    if (!queue.size) {
      global_default.requestAnimationFrame(execute);
    }
    queue.add(target);
  }
}
...
var suspense = /* @__PURE__ */ new WeakSet();
...
var gcList = /* @__PURE__ */ new Set();
function deleteEntry(entry) {
  if (!gcList.size) {
    global_default.requestAnimationFrame(() => {
      gcList.forEach((e) => {
...
function dispatch2(host, eventType, options = {}) {
  return host.dispatchEvent(
    new global_default.CustomEvent(eventType, { bubbles: false, ...options })
  );
}
function shadyCSS(fn, fallback) {
  const shady = global_default.ShadyCSS;
...
function walkInShadow(target, cb) {
  if (target.nodeType === global_default.Node.ELEMENT_NODE) {
    cb(target);
    if (target.shadowRoot) {
      walkInShadow(target.shadowRoot, cb);
    }
  }
  const walker = global_default.document.createTreeWalker(
    target,
    global_default.NodeFilter.SHOW_ELEMENT,
    null,
    false
  );
...
var HybridsRootElement = class extends global_default.HTMLElement {
  constructor() {
    super();
    const props = propsMap.get(this.constructor);
    for (let index2 = 0; index2 < props.length; index2 += 1) {
      const key = props[index2];
  ...
  connectedCallback() {
    unsuspend(this);
    const callbacks2 = callbacksMap.get(this.constructor);
    const list = [];
    for (let index2 = 0; index2 < callbacks2.length; index2 += 1) {
      const cb = callbacks2[index2](this);
      if (cb)
        list.push(cb);
    }
...
function property(key, desc) {
  ...
  const setAttr = (host, value) => {
    if (!value && value !== 0 || typeof value === "object" && value.toString() === void 0) {
  ...
  return {
    ...
    set: type !== "undefined" ? (host, value) => setAttr(host, transform(value)) : (host, value) => value,
    connect: type !== "undefined" ? (host, _, invalidate2) => {
      if (!host.hasAttribute(attrName) && host[key] === defaultValue) {
        setAttr(host, defaultValue);
      }
      return desc.connect && desc.connect(host, _, invalidate2);
    } : desc.connect,
...
function compile(hybrids, HybridsElement) {
  ...
  callbacksMap.set(HybridsElement, callbacks2);
  ...
  props.forEach((key) => {
    ...
    if (type === "function") {
      ...
      const attrName = camelToDash(key);
      const get3 = desc.get || ((host, value) => value);
      desc.get = (host, value) => {
        if (value === void 0) {
          value = desc.set(host, host.getAttribute(attrName) || value);
        }
        return get3(host, value);
      };
    }
    ...
    Object.defineProperty(HybridsElement.prototype, key, {
      get: function get3() {
        return get(this, key, desc.get);
      },
      set: desc.set && function set4(newValue) {
        set(this, key, desc.set, newValue);
      },
      enumerable: true,
      configurable: true
    });
    if (desc.observe) {
      callbacks2.unshift(
        (host) => observe(host, key, desc.get, desc.observe)
      );
    }
    if (desc.connect) {
      callbacks2.push((host) => {
        function invalidate2(options) {
          invalidate(host, key, {
            force: typeof options === "object" && options.force === true
          });
        }
        return desc.connect(host, key, invalidate2);
      });
    }
  });
  return HybridsElement;
...
function update(HybridsElement) {
  if (!updateQueue.size) {
    deferred.then(() => {
      walkInShadow(global_default.document.body, (node) => {
...
function define(hybrids) {
  ...
  const HybridsElement = global_default.customElements.get(hybrids.tag);
  if (HybridsElement) {
  ...
  global_default.customElements.define(hybrids.tag, compile(hybrids));
  return Object.freeze(hybrids);
}
...
function setupOfflineKey(config, threshold) {
  ...
  if (!clearPromise) {
    clearPromise = Promise.resolve().then(() => {
      const previousKeys = JSON.parse(global_default.localStorage.getItem(offlinePrefix)) || {};
      const timestamp = getCurrentTimestamp();
      Object.keys(previousKeys).forEach((k) => {
        if (!offlineKeys[k] && previousKeys[k] < timestamp) {
          global_default.localStorage.removeItem(k);
          delete previousKeys[k];
        }
      });
      global_default.localStorage.setItem(
        offlinePrefix,
...
function setupStorage(config, options) {
  ...
  if (result.offline) {
    ...
    try {
      const items = JSON.parse(global_default.localStorage.getItem(offlineKey)) || {};
      let flush;
        ...
        set(id, values) {
          ...
          if (!flush) {
            flush = Promise.resolve().then(() => {
              ...
              global_default.localStorage.setItem(offlineKey, JSON.stringify(items));
              flush = null;
            });
          }
          return values;
...
var stateSetter = (_, value, lastValue) => {
...
function setModelState(model, state, value = model) {
  set(model, "state", stateSetter, { state, value });
  return model;
}
...
function setupModel(Model, nested) {
  ...
  if (!config) {
        ...
        case "function":
          return (model) => {
            Object.defineProperty(model, key, {
              get() {
                return get(this, key, () => defaultValue(this));
              }
            });
          };
        case "object": {
              ...
              return (model, data, lastModel) => {
                if (hasOwnProperty.call(data, key)) {
                  if (!Array.isArray(data[key])) {
                    throw TypeError(
                      `The value for '${key}' property must be an array: ${typeof data[key]}`
                    );
                  }
          ...
          if (nestedConfig.enumerable || nestedConfig.external) {
            ...
            return (model, data, lastModel) => {
              ...
              if (resultModel) {
                ...
                Object.defineProperty(model, key, {
                  get() {
                    return get(this, key, () => get2(defaultValue, id));
                  },
                  enumerable: true
                });
              } else {
    ...
    config.create = function create2(data, lastModel) {
      if (data === null)
        return null;
      ...
      create(items, invalidate2 = false) {
        ...
        const result = items.reduce((acc, data) => {
          ...
          if (modelConfig.enumerable) {
            ...
            Object.defineProperty(acc, key, {
              get() {
                return get(this, key, () => get2(Model, id));
              },
              enumerable: true
            });
          }
    ...
    config.storage = Object.freeze({
      ...
      offline: modelConfig.storage.offline && {
        ...
        get: (id) => {
          ...
          return result ? result.map((item) => modelConfig.storage.offline.get(item)) : null;
        },
        set: (id, values) => {
...
function get2(Model, id) {
  ...
  return get(config, stringId, (h, cachedModel) => {
    ...
    if (config.contexts) {
      config.contexts.forEach((context2) => {
...
function clear(model, clearValue = true) {
  ...
  if (config) {
    ...
    invalidate(config, model.id, { clearValue, deleteEntry: true });
  } else {
...
function error(model, property2) {
  if (model === null || typeof model !== "object")
    return false;
  const state = getModelState(model);
  if (property2 !== void 0) {
    const errors = typeof state.error === "object" && state.error && state.error.errors;
    return property2 === null ? !errors && state.error : errors[property2];
  }
  return state.error;
}
...
function store(Model, options = {}) {
  ...
  if (!options.id && config.enumerable) {
    return {
      get(host, value) {
        const valueConfig = definitions.get(value);
        const id = valueConfig !== void 0 ? value.id : value;
        if (draft && (value === void 0 || value === null)) {
          const draftModel = draft.create({}, { id: void 0 });
          syncCache(draft, void 0, draftModel, false);
          return get2(Model, void 0);
        }
        return value ? get2(Model, id) : void 0;
      },
      set: (_, v) => v,
      connect: draft ? (host, key) => () => {
  ...
  return {
    get: (host, value) => {
      ...
      if (draft && !id && (value === void 0 || value === null)) {
        const draftModel = draft.create({});
        syncCache(draft, void 0, draftModel, false);
        return get2(Model, void 0);
      }
      if (config.enumerable && id === void 0)
        return void 0;
      const nextValue = get2(Model, id);
    ...
    set: draft && !config.enumerable ? (_, v) => v : void 0,
    connect: draft && config.enumerable ? (host, key) => () => {
      const model = host[key];
      if (model && model.id)
        clear(model, true);
    } : void 0
...
var configs2 = /* @__PURE__ */ new WeakMap();
var flushes = /* @__PURE__ */ new WeakMap();
var stacks = /* @__PURE__ */ new WeakMap();
var routers = /* @__PURE__ */ new WeakMap();
var rootRouter = null;
var entryPoints = /* @__PURE__ */ new Set();
var debug = false;
function setDebug(value = true) {
  debug = !!value;
}
var scrollMap = /* @__PURE__ */ new WeakMap();
var focusMap = /* @__PURE__ */ new WeakMap();
function saveLayout(target) {
  const focusEl = global_default.document.activeElement;
  focusMap.set(target, rootRouter.contains(focusEl) && focusEl);
  const map2 = /* @__PURE__ */ new Map();
  const rootEl = global_default.document.scrollingElement;
  map2.set(rootEl, { left: rootEl.scrollLeft, top: rootEl.scrollTop });
  walkInShadow(target, (el) => {
    if (el.scrollLeft || el.scrollTop) {
      map2.set(el, { left: el.scrollLeft, top: el.scrollTop });
    }
  });
  scrollMap.set(target, map2);
}
...
function restoreLayout(target) {
  const activeEl = global_default.document.activeElement;
  focusElement(
    focusMap.get(target) || (rootRouter.contains(activeEl) ? activeEl : rootRouter)
  );
  const map2 = scrollMap.get(target);
  if (map2) {
    const config = configs2.get(target);
    const state = global_default.history.state;
    const entry = state.find((e) => e.id === config.id);
    const clear2 = entry && entry.params.scrollToTop;
    map2.forEach((pos, el) => {
      el.scrollLeft = clear2 ? 0 : pos.left;
      el.scrollTop = clear2 ? 0 : pos.top;
    });
...
function setupBrowserUrl(browserUrl, id) {
  ...
  return {
    ...
    url(params, strict = false) {
      ...
      const url = new URL(temp, global_default.location.origin);
...
function setupViews(views, options, parent2 = null, nestedParent = null) {
  ...
  return views.map((hybrids) => {
    ...
    if (config && hasInStack(config, parent2)) {
      throw Error(
        `<${config.id}> cannot be in the stack of <${parent2.id}>, as it is an ancestor in the stack tree`
      );
    }
...
function getConfigById(id) {
  const Constructor = global_default.customElements.get(id);
...
function setupView(hybrids, routerOptions, parent2, nestedParent) {
  ...
  if (!config) {
    ...
    const callbacks2 = callbacksMap.get(Constructor);
    if (!nestedParent) {
      callbacks2.push(restoreLayout);
    }
    if (options.dialog) {
      callbacks2.push((host) => {
        const goBackOnEscKey = (event) => {
          if (event.key === "Escape") {
            ...
            global_default.history.go(-1);
          }
        };
        const focusDialog = () => {
          focusElement(host);
        };
        const prevActiveEl = global_default.document.activeElement;
    ...
    if (options.url) {
      ...
      browserUrl.paramsKeys.forEach((key) => {
        ...
        if (!desc || !desc.set) {
          throw Error(
            `'${key}' parameter from the url is not ${desc ? "writable" : "defined"} in <${id}>`
          );
        }
      });
    }
    ...
    callbacksMap.get(Constructor).unshift(
      (_) => observe(
        _,
        connect2,
        (host) => stateParams.reduce((acc, key) => {
          const value = mapUrlParam(host[key]).toString();
          if (value !== void 0 && value !== hybrids[key]) {
        ...
        (host, params, lastParams) => {
          ...
          global_default.history.replaceState(
    ...
    config = {
      ...
      ...browserUrl || {
        url(params) {
          const url = new URL("", global_default.location.origin);
          Object.keys(params).forEach((key) => {
            url.searchParams.append(key, mapUrlParam(params[key]));
          });
          return new URL(
            `${routerOptions.url}#@${id}${url.search}`,
            global_default.location.origin
          );
        },
        match(url) {
          const params = {};
...
function getEntryFromURL(url) {
  ...
  if (pathname && pathname.match(/^#@.+-.+/)) {
    ...
    url = new URL(`?${search}`, global_default.location.origin);
  }
  if (!config) {
...
function handleNavigate(event) {
  ...
  if (event.type === "click") {
    if (event.ctrlKey || event.metaKey)
      return;
    const anchorEl = event.composedPath().find((el) => el instanceof global_default.HTMLAnchorElement);
    if (anchorEl) {
      url = new URL(anchorEl.href, global_default.location.origin);
    }
  } else {
    url = new URL(event.target.action, global_default.location.origin);
  }
  if (url && url.origin === global_default.location.origin) {
    const entry = getEntryFromURL(url);
    if (entry) {
      event.preventDefault();
      dispatch2(rootRouter, "navigate", {
        bubbles: true,
        detail: { entry, url }
      });
    }
...
function resolveStack(host, state, options) {
  ...
  const reducedState = state.reduce((acc, entry, index2) => {
    if (index2 === 0 || state[index2 - 1].id !== entry.id || getConfigById(entry.id).multiple) {
      acc.push(entry);
    }
    return acc;
  }, []);
  const offset = stack.length - reducedState.length;
  stack = reducedState.map((entry, index2) => {
    const prevView = stack[index2 + offset];
    const config = getConfigById(entry.id);
    let nextView;
    if (prevView) {
      const prevConfig = configs2.get(prevView);
      if (config.id !== prevConfig.id || index2 === 0 && config.replace) {
...
function getEntryOffset(entry) {
  const state = global_default.history.state.reduce((acc, e, index2) => {
    let i2 = 0;
    while (e) {
      acc[i2] = acc[i2] || [];
      acc[i2][index2] = e;
  ...
  while (entry) {
    ...
    for (; j < state[i].length; j += 1) {
      ...
      if (e.id === entry.id) {
        if (config.multiple) {
          if (config.pathnameParams && config.pathnameParams.every(
...
function connectRootRouter(host, invalidate2, options) {
  function restoreScrollRestoration() {
    if (global_default.history.scrollRestoration === "manual") {
      global_default.history.scrollRestoration = "auto";
    }
  }
  function flush() {
    resolveStack(host, global_default.history.state, options);
    invalidate2();
    global_default.requestAnimationFrame(restoreScrollRestoration);
  }
  function navigateBack(offset, entry, nextUrl) {
    const stateLength = global_default.history.state.length;
    const targetEntry = global_default.history.state[offset];
    ...
    const replace = (popStateEvent) => {
      if (popStateEvent) {
        global_default.removeEventListener("popstate", replace);
        global_default.addEventListener("popstate", flush);
      }
      const state2 = global_default.history.state;
      const method = pushOffset ? "pushState" : "replaceState";
      const nextState = [entry, ...state2.slice(pushOffset ? 0 : 1)];
      if (pushOffset) {
        global_default.history.scrollRestoration = "manual";
      }
      global_default.history[method](nextState, "", nextUrl);
      flush();
    };
    if (offset) {
      global_default.removeEventListener("popstate", flush);
      global_default.addEventListener("popstate", replace);
      global_default.history.go(offset);
    } else {
      replace();
    }
  }
  function navigate(entry) {
    ...
    if (offset > -1) {
      ...
      global_default.history.scrollRestoration = "manual";
      global_default.history.pushState([entry, ...state2], "", url);
      flush();
    }
  }
  ...
  const state = global_default.history.state;
  if (!state) {
    const entry = getEntryFromURL(new URL(global_default.location.href)) || roots[0].getEntry();
    global_default.history.replaceState([entry], "", options.url);
    flush();
  } else {
...
function connectNestedRouter(host, invalidate2, options) {
  ...
  function getNestedState() {
    return global_default.history.state.map((entry) => {
      while (entry) {
...
function router(views, options) {
  options = {
    url: global_default.location.href.replace(/#.*$/, ""),
    params: [],
    ...options,
    views
  };
  const desc = {
    get: (host) => {
      const stack = stacks.get(host) || [];
      return stack.slice(0, stack.findIndex((el) => !configs2.get(el).dialog) + 1).reverse();
    },
    connect: (host, key, invalidate2) => {
    ...
    observe: debug && ((host, value, lastValue) => {
      const index2 = value.length - 1;
      const view = value[index2];
      if (lastValue && view === lastValue[index2])
        return;
      let config = configs2.get(host);
      let entry = global_default.history.state[0];
      let key = 0;
      while (config) {
        key += 1;
        entry = entry.nested;
      ...
      console.groupCollapsed(
        `[${host.tagName.toLowerCase()}]: navigated to <${entry.id}> ($$${key})`
      );
      Object.entries(entry.params).forEach(
        ([k, v]) => console.log(`%c${k}:`, "font-weight: bold", v)
      );
      console.groupEnd();
      global_default[`$$${key}`] = view;
    })
  };
  routers.set(desc, options);
  return desc;
}
var router_default = Object.freeze(
...
function removeTemplate(target) {
  if (target.nodeType !== global_default.Node.TEXT_NODE) {
...
function resolveArray(host, target, value, resolveValue3) {
  let lastEntries = arrayMap.get(target);
  const entries2 = value.map((item, index2) => ({
    id: hasOwnProperty.call(item, "id") ? item.id : index2,
    value: item,
    placeholder: null,
    available: true
  }));
  ...
  if (lastEntries) {
    ...
    entries2.forEach((entry) => ids.add(entry.id));
  ...
  const data = dataMap.get(target);
  for (let index2 = 0; index2 < entries2.length; index2 += 1) {
    ...
    if (matchedEntry) {
      ...
      if (matchedEntry.value !== entry.value) {
        resolveValue3(host, entry.placeholder, entry.value, matchedEntry.value);
      }
    } else {
      entry.placeholder = global_default.document.createTextNode("");
      previousSibling.parentNode.insertBefore(
        entry.placeholder,
        previousSibling.nextSibling
      );
      resolveValue3(host, entry.placeholder, entry.value);
    }
...
function resolveNode(host, target, value, lastValue) {
  const data = dataMap.get(target, {});
  if (lastValue)
    lastValue.parentNode.removeChild(lastValue);
...
function resolveValue(host, target, value, lastValue) {
  ...
  if (Array.isArray(value)) {
    type = "array";
  } else if (value instanceof global_default.Node) {
    type = "node";
  }
  ...
  return (host, target, value, lastValue) => {
    ...
    if (value) {
      ...
      target.addEventListener(
        eventType,
        callback,
        value.options !== void 0 ? value.options : false
      );
    }
  };
}
...
function normalizeValue(value, set4 = /* @__PURE__ */ new Set()) {
  if (Array.isArray(value)) {
    value.forEach((className) => set4.add(className));
  } else if (value !== null && typeof value === "object") {
    Object.keys(value).forEach((key) => value[key] && set4.add(key));
  } else {
    set4.add(value);
  }
  return set4;
...
function resolveStyle(host, target, value) {
  ...
  const nextMap = Object.keys(value).reduce((map2, key) => {
  ...
  switch (attrName) {
    ...
    default: {
      ...
      return (host, target, value) => {
        isProp = isProp || !isSVG && !(target instanceof global_default.SVGElement) && propertyName in target;
        if (isProp) {
...
var preparedTemplates = /* @__PURE__ */ new WeakMap();
function applyShadyCSS(template, tagName) {
  if (!tagName)
    return template;
  return shadyCSS((shady) => {
    let map2 = preparedTemplates.get(template);
    if (!map2) {
      map2 = /* @__PURE__ */ new Map();
      preparedTemplates.set(template, map2);
    }
    let clone = map2.get(tagName);
    if (!clone) {
      clone = global_default.document.createElement("template");
      ...
      Array.from(styles).forEach((style) => {
        ...
        for (let i = 0; i < count; i += 1) {
          style.parentNode.insertBefore(
            global_default.document.createTextNode(getPlaceholder()),
            style
          );
        }
      });
...
function createSignature(parts, styles) {
  let signature = parts.reduce((acc, part, index2) => {
    if (index2 === 0) {
      return part;
    }
    if (parts.slice(index2).join("").match(/^\s*<\/\s*(table|tr|thead|tbody|tfoot|colgroup)>/)) {
      return `${acc}<!--${getPlaceholder(index2 - 1)}-->${part}`;
    }
    return acc + getPlaceholder(index2 - 1) + part;
  }, "");
  if (styles) {
...
function replaceComments(fragment) {
  const iterator = global_default.document.createNodeIterator(
    fragment,
    global_default.NodeFilter.SHOW_COMMENT,
    null,
    false
  );
  let node;
  while (node = iterator.nextNode()) {
    if (PLACEHOLDER_REGEXP_EQUAL.test(node.textContent)) {
      node.parentNode.insertBefore(
        global_default.document.createTextNode(node.textContent),
        node
      );
      node.parentNode.removeChild(node);
    }
...
function createInternalWalker(context2) {
  let node;
  return {
    get currentNode() {
      return node;
    ...
    nextNode() {
      if (node === void 0) {
        ...
        while (!node && parentNode !== context2) {
...
function createExternalWalker(context2) {
  return global_default.document.createTreeWalker(
    context2,
    global_default.NodeFilter.SHOW_ELEMENT | global_default.NodeFilter.SHOW_TEXT,
    null,
    false
  );
}
var createWalker = typeof global_default.ShadyDOM === "object" && global_default.ShadyDOM.inUse ? createInternalWalker : createExternalWalker;
...
function beautifyTemplateLog(input, index2) {
  const placeholder = getPlaceholder(index2);
...
function compileTemplate(rawParts, isSVG, styles) {
  const template = global_default.document.createElement("template");
  const parts = [];
  ...
  if (isSVG) {
    ...
    Array.from(svgRoot.childNodes).forEach(
      (node) => template.content.appendChild(node)
    );
  }
  ...
  while (compileWalker.nextNode()) {
    ...
    if (node.nodeType === global_default.Node.TEXT_NODE) {
          ...
          results.reduce(
            ...
            if (index2 === 0) {
              ...
              currentNode = currentNode.parentNode.insertBefore(
                global_default.document.createTextNode(part),
      ...
      if (node.nodeType === global_default.Node.ELEMENT_NODE) {
        if (probablyDevMode) {
          const tagName = node.tagName.toLowerCase();
          if (tagName.match(/.+-.+/) && !global_default.customElements.get(tagName) && !notDefinedElements.includes(tagName)) {
        ...
        Array.from(node.attributes).forEach((attr) => {
          ...
          if (equal) {
            ...
            if (results) {
              ...
              results.forEach((placeholder, index2) => {
                ...
                parts[id] = [
                  ...
                  (host, target, attrValue) => {
                    ...
                    if (results.length === 1 || index2 + 1 === results.length) {
                      isProp = isProp || !isSVG && !(target instanceof global_default.SVGElement) && name in target;
                      if (isProp) {
...
${beautifyTemplateLog(signature, -1)}`
  ...
  return function updateTemplateInstance(host, target, args, styleSheets) {
    ...
    if (template !== data.template) {
      if (data.template || target.nodeType !== global_default.Node.TEXT_NODE) {
        removeTemplate(target);
      }
      data.prevArgs = null;
      const fragment = global_default.document.importNode(
    ...
    if (styleSheets) {
      ...
      styleSheets = styleSheets.map((style) => {
        if (style instanceof global_default.CSSStyleSheet)
          return style;
        let styleSheet = styleSheetsMap.get(style);
        if (!styleSheet) {
          styleSheet = new global_default.CSSStyleSheet();
    ...
    data.prevArgs = args;
    for (let index2 = 0; index2 < data.markers.length; index2 += 1) {
      if (prevArgs && prevArgs[index2] === args[index2])
        continue;
      const [node, marker] = data.markers[index2];
      try {
        marker(host, node, args[index2], prevArgs && prevArgs[index2]);
      } catch (error2) {
        console.error(
          `Following error was thrown when updating a template expression in ${stringifyElement(
            host
          )}
${beautifyTemplateLog(signature, index2)}`
        );
        throw error2;
      }
    }
...
var storeValues = /* @__PURE__ */ new WeakMap();
function set3(property2, valueOrPath) {
  if (!property2) {
    throw Error(
      `The first argument must be a property name or an object instance: ${property2}`
    );
  }
  if (typeof property2 === "object") {
    ...
    const store2 = storePointer.get(property2);
    if (!store2) {
      throw Error("Provided object must be a model instance of the store");
    }
    if (valueOrPath === null) {
      return () => {
        store2.set(property2, null);
      };
    }
    return (host, event) => {
      resolveValue2(event, (value) => {
        const values = storeValues.get(property2);
        if (!values) {
          global_default.requestAnimationFrame(() => {
            const result = storeValues.get(property2);
            storeValues.delete(property2);
            store2.set(property2, result).catch(() => {
            });
          });
        }
        storeValues.set(property2, {
          ...values,
  ...
  let fn = stringCache.get(property2);
  if (!fn) {
    fn = (host, event) => {
      resolveValue2(event, (value) => {
...
var templatesMap = /* @__PURE__ */ new Map();
  ...
  css(parts, ...args) {
    stylesMap.set(
      ...
      (stylesMap.get(this) || []).concat(
        parts.reduce(
          (acc, part, index2) => `${acc}${part}${args[index2] || ""}`,
          ""
        )
      )
    );
...
function create(parts, args, isSVG) {
  const createTemplate = (host, target = host) => {
    ...
    if (isSVG)
      id += SVG_PLACEHOLDER;
    let render3 = templatesMap.get(id);
    if (!render3) {
      render3 = compileTemplate(parts, isSVG, !hasAdoptedStyleSheets && styles);
      templatesMap.set(id, render3);
    }
    render3(host, target, args, hasAdoptedStyleSheets && styles);
  };
...
Object.assign(html, helpers_exports);
Object.assign(svg, helpers_exports);
...
// ../node_modules/tdesign-icons-view/dist/index.js
...
var developHTML = function developHTML2(name, iconName) {
...
var designHTML = function designHTML2(name, svgString) {
