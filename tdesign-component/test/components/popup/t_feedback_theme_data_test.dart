...
void main() {
  test('Feedback ThemeExtension merge/copyWith/lerp contracts', () {
    ...
    expect(
        const TDialogThemeData(width: 10)
            .merge(const TDialogThemeData(elevation: 3))
            .width,
        10);
    expect(
      const TDropdownThemeData(barHeight: 10)
          .merge(const TDropdownThemeData(barHeight: 20))
          .barHeight,
      20,
    );
    expect(
        const TLoadingThemeData(duration: 10)
            .merge(const TLoadingThemeData(axis: Axis.horizontal))
            .duration,
        10);
    expect(
        const TMessageThemeData(backgroundColor: Colors.black)
            .merge(const TMessageThemeData(elevation: 2))
            .elevation,
        2);
    expect(
        const TNoticeBarThemeData(height: 10)
            .merge(const TNoticeBarThemeData(height: 20))
            .height,
        20);
    expect(
        const TPopoverThemeData()
    ...
    expect(
        const TPopupThemeData(panelRadius: 8)
            .merge(const TPopupThemeData(panelRadius: 12))
            .panelRadius,
        12);
    expect(
        const TRefreshThemeData()
            .merge(const TRefreshThemeData(backgroundColor: Colors.red))
            .backgroundColor,
        Colors.red);
    expect(
    ...
    expect(
        const TToastThemeData(maxWidth: 10)
            .merge(const TToastThemeData(maxWidth: 20))
            .maxWidth,
        20);
  });
}
