...
# picker
dart run tdesign_flutter_tools:main generate --folder "$PARENT_DIR/lib/src/components/picker" --name TPicker,TPickerOption,TPickerValue,TPickerColumns,TPickerLinked,TPickerItems,TPickerKeys,ItemBuilderType --folder-name picker --output "$PARENT_DIR/example/assets/api/" --only-api --get-comments
# date-time-picker
