...
///
/// 精简版，仅包含 TCalendar 时间选择器所需功能（`List` 数据源 +
/// `FixedExtentScrollController`），与对外组件 `TDateTimePicker` /
/// `DateTimePickerDataHelper` 为**平行实现**。
///
/// **收敛方向**：新需求与缺陷修复应优先落在 `DateTimePickerDataHelper` 与
/// `TDateTimePicker`；日历侧后续可改为由同一数据层生成列选项，再保留本模型仅作
/// 滚动状态适配，以减少双轨逻辑分歧（无明确排期，迁移前若行为需一致请同步修
/// 两处）。
class DatePickerModel {
  ...
  List<int> get years {
    ...
    final end =
        (dateEnd != null && dateEnd!.isNotEmpty) ? dateEnd![0] : 2100;
