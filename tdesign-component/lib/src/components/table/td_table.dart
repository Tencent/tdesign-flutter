...
class TDTableState extends State<TDTable> {
  ...
  Widget _getTableHeader(BuildContext context) {
    ...
    var start = 0;
    var fixedLeftCells = <Widget>[], cells = <Widget>[], fixedRightCells = <Widget>[];
    ...
    for(var i = 0; i < fixedRightCol.length; i++) {
      ...
      if(fixedRightCol[i].width != null) {
        fixedRightCells.add(SizedBox(width: fixedRightCol[i].width, child: cell));
      }else {
    ...
    for(var i = 0; i < widget.data!.length; i++) {
      ...
      for(var j = 0; j < headerCol.length; j++) {
        var cell = _getCell(
          ...
          data,
          i,
          j == (fixedLeftCol.length - 1) || j == (fixedLeftCol.length + fixedNonCol.length),
        );
      ...
      cells.add(
        Container(
          color: (widget.stripe ?? false) && i % 2 == 0 ? const Color(0xffF3F3F3) : Colors.white,
  ...
  /// 获取单元格
  Widget _getCell(TDTableCol col, bool isHeader, dynamic data, int index, bool fixedBorder){
    ...
    var topBorder = BorderSide.none, rightBorder = BorderSide.none, leftBorder = BorderSide.none;
          ...
          child: Padding(
            ...
            child: SizedBox(
              ...
              child: Align(
                ...
                child: _getCellText(col, title, ellipsis, isHeader, sortable, index),
              ),
            ),
          )
      ),
  ...
  /// 获取单元格内容
  Widget _getCellText(TDTableCol col, String title, bool ellipsis, bool isHeader, bool sortable, int index) {
    ...
    var titleWidget = TDText(
        ...
        overflow: overflow,
        style: TextStyle(
          color: isHeader ? TDTheme.of(context).fontGyColor3 : TDTheme.of(context).fontGyColor1,
          fontSize: 14,
          height: 1,
          letterSpacing: 0,
        )
              ...
              child: GestureDetector(
                ...
                child: CustomPaint(
                  ...
                  painter: ChevronPainter(
                    upColor: (_sortable == true) && (_sortKey == col.colKey) ? selectColor : unSelectColor,
                    downColor: (_sortable == false) && (_sortKey == col.colKey) ? selectColor : unSelectColor,
                  ),
                ),
              ),
            ),
  ...
  Widget _getFixedTable(BuildContext context) {
    ...
    // 生成左侧固定列
    var fixedLeftCols = _getVerticalCell(fixedLeftCol, fixedLeftTitle, cellWidth);
    // 生成非固定列
    var fixedNonCols = _getVerticalCell(fixedNonCol, fixedNonTitle, cellWidth);
    // 生成右侧固定列
    var fixedRightCols = _getVerticalCell(fixedRightCol, fixedRightTitle, cellWidth);

    // 固定列宽度
    var fixedCellsWidth = (fixedLeftCol.length + fixedRightCol.length) * cellWidth;
  ...
  /// 竖向生成单元格
  List<Widget> _getVerticalCell(List<TDTableCol> cols, List<List<String>> titles, double cellWidth) {
    var rows = <Widget>[];
    for(var i = 0; i < titles.length; i++) {
      var cells = <Widget>[];
      for(var j = 0; j < titles[i].length; j++) {
        var col = cols[i];
        var cell = _getCell(col, j == 0, j == 0 ? '' : widget.data?[j-1], i, i == titles.length - 1);
  ...
  Widget build(BuildContext context) {
    ...
    // 自定义表格宽度 默认屏幕宽度
    var width = widget.width ?? MediaQuery.of(context).size.width;
    var fixedCols = [..._getCol(TDTableColFixed.left), ..._getCol(TDTableColFixed.right)];
