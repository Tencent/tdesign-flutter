  ...
  for (final mode in [ThemeMode.light, ThemeMode.dark]) {
    testWidgets('indexes letter ${mode.name} opened golden', (tester) async {
      ...
      await expectLater(
        find.byType(Overlay),
        matchesGoldenFile(
          'goldens/indexes_letter_opened_${mode.name}.png',
        ),
      );
    ...
    testWidgets('indexes number ${mode.name} opened golden', (tester) async {
      ...
      await expectLater(
        find.byType(Overlay),
        matchesGoldenFile(
          'goldens/indexes_number_opened_${mode.name}.png',
        ),
      );
      await disposeDemoPage(tester);
    }, tags: 'golden');
    ...
    testWidgets('indexes capsule ${mode.name} opened golden', (tester) async {
      ...
      await expectLater(
        find.byType(Overlay),
        matchesGoldenFile(
          'goldens/indexes_capsule_opened_${mode.name}.png',
        ),
      );
      await disposeDemoPage(tester);
    }, tags: 'golden');
    ...
    for (final entry in const {
      ...
      testWidgets('indexes ${entry.key} ${mode.name} golden', (tester) async {
        ...
        await expectLater(
          find.byType(Overlay),
          matchesGoldenFile(
            'goldens/indexes_${entry.key}_${mode.name}.png',
          ),
        );
        await disposeDemoPage(tester);
      }, tags: 'golden');
