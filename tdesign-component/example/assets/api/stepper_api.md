...
| textStyle | TextStyle? | - | 输入文字样式。 在继承 DefaultTextStyle 和 ThemeData.textTheme 后合并；非空字段可覆盖 默认字号、行高及 `foregroundColor`。仅覆盖字号时会按最终字号重新计算 默认行高倍数；显式设置的 `TextStyle.height` 始终优先。 |
