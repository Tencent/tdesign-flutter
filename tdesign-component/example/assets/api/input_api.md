...
| layout | TInputLayout | TInputLayout.horizontal | 标签与输入区的排布方式，默认横向（label 在左）。 |
| status | TInputStatus | TInputStatus.normal | 输入框状态，影响边框与 `tips` 文本颜色。 |
| tips | String? | - | 输入区下方提示文本，颜色随 `status`。 |
| align | TInputAlign? | - | 输入内容位置；未传时回退到 `textAlign`。 |
| clearable | bool? | - | 是否可清空；为 false 时不显示内置清除按钮。 |
| clearTrigger | TInputClearTrigger | TInputClearTrigger.always | 内置清除按钮的触发方式。 |
