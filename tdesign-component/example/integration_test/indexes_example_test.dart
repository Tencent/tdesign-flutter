...
void main() {
  ...
  testWidgets('Indexes real app navigation and touch interaction', (
    ...
    final basicTrigger = find.byKey(const ValueKey('indexes-basic-trigger'));
    await tester.ensureVisible(basicTrigger);
    await tester.tap(basicTrigger);
    await tester.pumpAndSettle();
    ...
    await binding.takeScreenshot('indexes-basic-initial-b');

    await tester.tap(indexSemantics('G'));
    await tester.pumpAndSettle();
    expect(indexSemantics('G', selected: true), findsOneWidget);
    expect(find.text('广州'), findsOneWidget);
    await binding.takeScreenshot('indexes-basic-selected-g');

    navigator.pop();
    await tester.pumpAndSettle();
    final customTrigger = find.byKey(const ValueKey('indexes-custom-trigger'));
    await tester.ensureVisible(customTrigger);
    await tester.tap(customTrigger);
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('indexes-custom-panel')), findsOneWidget);
    ...
    await binding.takeScreenshot('indexes-custom-selected-10');

    navigator.pop();
    await tester.pumpAndSettle();
    final provider = tester.element(customTrigger).read<ThemeModeProvider>();
    final previousMode = provider.themeMode;
    provider.themeMode = ThemeMode.dark;
    await tester.pumpAndSettle();
    await tester.tap(customTrigger);
    await tester.pumpAndSettle();
    await binding.takeScreenshot('indexes-custom-dark');
