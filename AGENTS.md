...
2. **提交 PR 遵守模板**：按 `.github/PULL_REQUEST_TEMPLATE.md` **完整保留原模板结构**（所有选项含未勾选的 `[ ]`、所有 HTML 注释原样保留），只打勾 / 填写、不删减；PR 描述「更新日志」须按条分开列写（`tdesign-component/CHANGELOG.md` 由 CLI 自动生成，无需人工维护）。格式细则见 [`CONTRIBUTING.md`](./CONTRIBUTING.md)。
