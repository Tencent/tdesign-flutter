...
- 当用户提供 GitHub issue 链接并要求修复时，优先使用 skill **`issue-fix-entry`** 作为一键入口，再按 **`issue-fix-workflow`** 展开细节；并配合 issue workflow 相关 rules、agents、模板与 `scripts/issue-workflow/` 下的辅助脚本。
