...
2. **提交 PR 遵守模板**：按 `.github/PULL_REQUEST_TEMPLATE.md` 逐项填写并补充完整内容。
3. **关联相关 Issue**：有相关 Issue 时，在 PR 正文以 `close #xx`（如 `close #22`）形式携带关联。
4. **Flutter 双版本兼容**：同时兼容 `flutter@3.32.0` 与 `flutter@latest`。
5. **breaking change 分析**：组件改动时判断是否改变公开 API 签名 / 默认行为 / 删除能力。
