...
- 声明式状态优先使用不可变参数和 nullable callback；内容扩展使用 Widget/builder；原始颜色、字号、圆角、间距等样式值进入 token/Theme，实例 API 只保留必要的语义选择器或完整样式逃逸入口；Controller 只处理无法由声明式重建表达的跨树命令或生命周期协调；
...
- 实例参数与 ThemeExtension 在没有真实子树默认需求时重复保存同一个状态或选择器；
- 实例级颜色、字号、圆角等零散标量与 Theme 重复暴露；完整的 Flutter 样式对象逃逸入口不在此列，但必须有明确覆盖优先级；
...
区分“默认样式从哪里取得”和“运行时谁优先覆盖”：默认视觉应由 TDesign token 或组件 Theme 的语义值提供，但解析优先级仍遵循上面的覆盖链，不能让 token 反向遮蔽实例参数或上层主题。

- Review 新增或修改的颜色、字体、行高、字重、圆角、阴影、间距、内边距、宽高、指示器尺寸、边框和描边；默认实现不得散落直接决定可见样式的常量。
- 优先复用语义匹配的现有 token。不得只因数值相等就挪用无关 token；能由同一语义下的字体、行高和间距自然计算时可以组合，否则应评估组件 Theme 或新的稳定 token，而不是伪造映射。
