...
视觉优先级按字段生效，不能因 `TextStyle`、`IconThemeData`、`ColorScheme` 或组件 Theme 对象非空，就把对象内全部字段视为调用方显式覆盖。Flutter 默认主题生成、`copyWith` / `apply` 的补全值、TDesign Theme 投影和继承链带入的值仍属于默认来源，不得反向遮蔽 TDesign 语义 token 或组件状态样式。

- 调用方只配置字体家族、字体回退、字号或某个颜色时，未配置字段继续按原优先级解析；局部配置不得顺带改写选中、未选中、禁用、错误等状态的默认字重、颜色或透明度；
- 组件内部产生的默认样式不能通过实例 `style`、便利参数或其他高优先级入口回传给子组件，否则会把默认值伪装成实例显式覆盖；
- 组合组件复用子组件时，应由共享解析器或明确的字段级合并实现优先级，不在每个消费组件复制一套近似判断；
- 现有主题基础设施无法区分显式字段与自动补全值时，优先修正共享解析边界；不得为绕过解析问题增加组件公开样式参数。

修改主题解析后，聚焦测试应覆盖默认 TDesign token、自定义 token、组件 ThemeExtension、适用的 Flutter 标准主题、子树继承主题和局部主题配置，并在适用时覆盖各语义状态及全部共享消费者。测试须读取最终布局或绘制结果；只检查中间 Theme 对象不足以证明优先级正确。

### ThemeExtension 默认值与插值

ThemeExtension 的配置值、运行时有效值和插值值必须使用同一默认语义。nullable 字段以 `null` 表示内置默认值时，`lerp` 不能把 `null` 当作 `0`、空颜色或其他无关初值。

- 单侧为 `null` 时，使用该侧的运行时有效默认值参与插值；两侧均为 `null` 时保持 `null`，避免把未配置状态固化为显式默认值；
- 连续字段按有效值插值；离散字段采用明确且稳定的切换点；每个中间结果都必须满足构造断言和组件布局、绘制约束；
- 默认值变更时同步检查消费端回退、`copyWith`、`lerp`、dartdoc 和测试，不能在多处保留不同默认值；
- 测试覆盖显式值之间、`null` 到显式值、显式值到 `null`、两侧均为 `null`，并至少检查 `t = 0`、中间值、`t = 1` 以及动态 Theme 切换后的实际呈现。
