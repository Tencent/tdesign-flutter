...
description: Review TDesign Flutter 组件的公开 Demo、Example 迁移、API 收敛、Flutter 设计模式、主题、测试和 Golden 证据。以小程序公开 Demo 作为可见效果参考，但不机械映射其 props/events；适用于组件对齐、Example 重构和 PR Review。
---
...
- 枚举全部取值、默认映射和有意义的组合；不能要求用户从多个字段注释自行拼出行为矩阵；
- 便捷组件与底层组件的默认映射、能力边界及何时应改用底层组件；
- 一个最小、真实、可复制的典型示例；关键配置不能只隐藏在未展示的私有 helper 中。

源码 dartdoc 是文档事实源，但必须运行仓库 API 生成器并检查实际生成产物；只在 IDE 注释中清楚、生成 API 中丢失仍判为未完成。特别核对生成器选取的是构造参数还是字段注释，避免同一参数的短注释覆盖完整契约。生成成功或产物同步只证明生成链路可用，不证明内容覆盖完整。

文档只描述用户可观察的公开契约，不暴露私有类、内部算法或仅服务 Demo 的常量。本次变更涉及的公开行为若缺少上述说明，或生成示例无法让用户复现，属于阻塞问题；与当前范围无关的历史文档缺口单独登记，不扩大无关 PR。不要用仅匹配关键词或固定文案的测试冒充文档完整性验证。
...
拆分或合并 Example 文件、移动 builder / 私有状态 / 辅助方法 / 数据、调整 `ExampleModule` / `ExampleItem`、修改 `exampleCodeGroup` / `methodName` / `@ExampleCode` 映射，均按 Example 迁移处理。迁移的首要目标是证明公开场景、行为、代码面板和视觉状态没有丢失，不能只证明新文件可以编译。

迁移前先在旧结构上建立一份共享的公开场景契约。每个场景至少记录稳定 id、公开分组与标签、源码入口、关键初始状态、必须执行的操作、预期结果、代码片段入口，以及 Golden 策略。Golden 策略必须明确为以下一种，不允许留空：

- `page`：静态页面或禁用入口只需由整页快照覆盖；
- `opened`：弹层、菜单、抽屉等必须真实操作后保存展开态；
- `postAction`：选择、翻页、输入、确认等会产生独立视觉结果时保存操作后状态；
- `functionalOnly`：没有独立视觉结果，只做功能断言；必须写明理由，不能作为漏图兜底。

覆盖完整性必须由测试自动证明，不能依靠文件数量、命名约定或人工清单：

1. 从渲染后的完整页面收集公开分组、场景入口、顺序与数量，并与共享场景契约做**双向集合及顺序校验**；只断言契约中的条目“都能找到”仍会漏掉页面新增项。
2. 功能测试声明其覆盖的场景 id；测试断言公开场景集合与“已覆盖功能场景”集合相等。禁用入口也要执行真实点击并验证无操作结果。
3. Golden 测试声明其覆盖的 `场景 id + 状态 + theme`；测试断言需要 `page` / `opened` / `postAction` 的期望集合，与实际注册的 Golden 集合完全相等。`functionalOnly` 必须单独进入显式豁免集合，并校验它与功能测试映射一致。
4. 覆盖契约、功能映射和 Golden 映射可以共享类型与常量，但页面结构断言必须从实际渲染结果取值；不得让多个测试只引用同一份手写列表而不与运行页面核对，否则同时漏项仍会全部通过。

若迁移前发现 Golden 或功能测试不完整，先在**迁移前实现**上补齐缺失基线并无更新参数复跑，再开始移动代码；建议将“补基线”和“纯迁移”分成独立提交，便于确认迁移没有借更新基线掩盖回归。无法先补齐时必须把缺口列为阻塞项，不得宣称迁移已由快照保护。

纯文件迁移、私有符号重命名或等价重构默认不应产生视觉变化：先运行既有 Golden 且不得带 `--update-goldens`；任何像素差异、基线缺失或快照数量减少都先按迁移回归处理。只有确认需求本身包含有意视觉变化时，才按下方 Golden 更新流程处理，并把视觉修改与迁移证据分开说明。

迁移完成后同时验证：所有迁移文件参与实际编译；生成器及 `--check` 通过；每个代码面板入口仍指向真实运行源码；私有数据、状态、回调、辅助方法和生命周期依赖没有因拆文件而从展示片段或运行路径中丢失。Golden 只负责视觉回归，不能替代这些编译、交互和代码片段检查。
...
- 建立可审计的 Golden 覆盖矩阵，逐项记录“公开 `ExampleItem` / 场景 → 初始或操作后稳定可见状态 → 真实触发步骤 → 是否存在实质视觉差异 → light/dark Golden → 功能测试 → 未单独截图时的等价覆盖依据”。场景清单从实际公开页面和目标设计取得，不能只从现有 Golden 文件反推。
- 主页或初始页面 Golden 不能替代点击、选择、翻页、展开、滚动、输入、加载完成等操作后才出现的稳定视觉状态。每个存在实质视觉差异且属于当前验收范围的稳定状态都必须有对应 Golden；仅回调结果、不可稳定捕获的瞬时动画或没有视觉差异的交互用功能测试覆盖，并在矩阵中说明。
- 多个场景只有在使用相同渲染路径、关键配置和布局结果时才可共用代表性 Golden，并记录“被代表场景 → 代表 Golden → 等价依据”。文案长度、图标、状态、按钮模式、分页、溢出或尺寸会改变布局时不得视为等价。不要为了数量机械复制相同快照，也不能以“看起来接近”省略差异场景。
- 将公开场景集合、Golden 登记集合和功能测试集合做集合核对。新增、删除或改名公开场景时同步更新矩阵与登记；无解释的缺失场景、操作后状态或孤立基线属于阻塞问题。只有矩阵无遗漏、等价关系有证据且登记测试实际执行时，才能宣称 Golden 覆盖完整；否则明确报告为代表性或部分覆盖。
