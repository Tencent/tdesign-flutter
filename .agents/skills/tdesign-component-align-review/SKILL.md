...
2. **稳定检查点**：实现者确认本批次代码已基本定型后，先检查 `git diff --check`、生成产物 `--check`（若适用）和受影响功能测试，再一次性运行本次范围所需的完整非视觉门禁。把测试结果关联到当前源码 / diff 指纹；之后任何生产源码、Demo、Theme、资源或测试入口变更都会使该检查点失效，必须重新走对应层级。
