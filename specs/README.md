...
> **「是否需要 Spec」与「是否需要更新日志」是两件独立的事**：Spec 看**改动复杂度 / 是否碰公共契约**（面向开发者），更新日志看**用户感不感知得到**（面向用户），两者不能互相推导。例如**行为不变的纯内部重构**：属于重构，**需要 Spec**（Review 结合实际改动判定），但用户感知不到，**不写更新日志**，勾选「本条 PR 不需要纳入 Changelog」——不要因"不写日志"就跳过 Spec。完整三种情况见 [`CONTRIBUTING.md`](../CONTRIBUTING.md)「PR 更新日志规范」第 3 条。
