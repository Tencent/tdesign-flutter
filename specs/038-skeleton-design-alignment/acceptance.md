...
- 基线：已合入 `origin/develop@c2f9ef5e`
...
| 命令 | 结果 | 备注 |
| --- | --- | --- |
| `flutter test test/components/skeleton/t_skeleton_test.dart` | 17/17 通过 | Flutter 3.32.0；覆盖构造路径互斥与闪烁动画 0s / 1s / 2s 关键帧；Flutter 3.47.0 待远端 CI 复验 |
...
| `flutter test --coverage test/components/skeleton/t_skeleton_test.dart` | 140/141，99.29% | Skeleton 生产目录 LH/LF |
...
- [x] 在 Android 真机运行并停留在“02 组件动效”，录制 4s 画面观察闪烁动画完整往返两个周期。
- [x] 核对浅色、深色完整页面 Golden：375×1991，中文字体正常，无缺字方框。
