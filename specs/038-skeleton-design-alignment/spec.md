...
- 普通构造函数使用非空 `variant` 选择四种内置结构；`TSkeleton.custom` / `layout` 是自定义结构的唯一入口。
...
- 默认多行间距为 16dp，文本首行内部间距为 16dp；默认占位色使用 secondary-container token。
...
是。普通构造函数的 `variant` 参数收敛为非空类型；原先显式传入 `TSkeleton(variant: null)` 的非法调用需改为 `TSkeleton()`，自定义结构改用 `TSkeleton.custom(layout: ...)`。

## 验收标准

- [x] Demo 分组、说明、顺序和实例符合移动端设计与公开示例。
- [x] 组件测试覆盖预设、自定义布局、视觉优先级、Theme 插值、延迟和动画生命周期。
- [x] Flutter 3.32.0 与 latest 的功能测试和严格 analyze 通过。
- [x] Skeleton 生产源码覆盖率 LH/LF 不低于 95%。
- [x] 固定 Linux Flutter 3.32.0 的 light/dark Golden 可复现。
- [x] 最终运行 Web Demo，观察并比对渐变与闪烁动画状态。
