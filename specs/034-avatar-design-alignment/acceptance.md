...
- [x] 2026-09-18 按主设计稿节点 `25054:18489` 重新复核：圆点为 `8×8`，三个徽标顶边约为头像顶边 `-2px`；Flutter Demo 使用 `right: -4, top: -2`，不再把圆点局部放大为 10px
- [x] 明暗 Golden 中本次变更的头像组裁剪、边框与 `+5` 溢出文案符合设计稿
- [x] 小程序公开能力逐项核对；图片、徽标继续通过 Flutter Widget 组合，组级 API 收敛为布局参数与成员自身形状

原 PR 使用的 `10×10, top: -4` 结论与当前主设计稿不一致；本轮以主设计稿节点和导出原图坐标为准，修正为默认 8px Dot 与 `top: -2`。仓库 Golden 需在规定的 Linux/Flutter/字体环境中单独更新并严格复跑。
