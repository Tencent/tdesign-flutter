...
- 横向 Demo 使用通栏容器背景；Radio 自身提供上下各 `spacer16` 的内容留白，外层只保留水平 `spacer16`，不得叠加第二层垂直留白。
- 块级 Radio 根据三档目标高度动态计算上下内边距；分割线行使用 `bgColorContainer` 承接左侧透明区域，实际线条不带外边距并从正文起点开始，避免页面底色造成视觉通栏，均对齐小程序默认块级布局。
- 未选中且禁用的圆形指示器使用 `componentBorderColor` 描边和 `bgColorComponentDisabled` 填充；默认浅色主题分别对应 `#DCDCDC` 与 `#EEEEEE`，不硬编码色值。
- 纵向和横向卡片的文案区域使用上下各 `spacer16`，并在卡片边框内垂直居中。
