...
人工核对结论：组件类型已收敛为“全部产品 + 默认排序”和同栏 1/2/3 列多选，组件状态为两个禁用菜单，入口顺序与小程序公开 Demo 一致；自定义面板、向上展开等能力移入独立的 Flutter 额外能力分组，未新增公共 API。单选展开态由点击触发后截图，禁用和方向交互另由 Widget 测试验证。
