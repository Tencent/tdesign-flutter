...
| 命令 | 结果 | 备注 |
| --- | --- | --- |
| 定向组件回归与覆盖率 | 73/73 通过 | Navbar 行为、Theme 优先级与 `TNavBarThemeData` 回归；`TNavBar` / `TNavBarThemeData` 生产 LH/LF = 174/178 = 97.75% |
| Demo 独立行为 | 3/3 通过 | 安全区、Figma H5 组合、图片尺寸、搜索与操作反馈，以及 Material 主题污染隔离 |
...
- [x] 最终 light/dark Golden 逐张核对；补齐 Navbar 独立 CJK 子集，确认无缺字方框、裁切或状态错位
...
- 最终 app/lib 代码通过 `flutter run` 执行 uppercase `R`，返回 `Restarted application in 923ms`。
- CNB 首轮 Flutter 3.32 全量视觉回归暴露公共 ExamplePage 壳层隐式继承 Navbar 新默认标题样式，造成多个无关组件 Golden 漂移；最终通过显式 shell leading/title 样式隔离并以 ActionSheet 12 个 Golden 复核。
...
- 设备集成在最终源码重跑：`flutter test integration_test/navbar_example_test.dart -d 40302eeb`，1/1 通过，覆盖真实路由、操作反馈、搜索与主题切换。
- 集成测试结束后重新执行 `flutter build apk --debug`，通过手机“USB 安装提示”点击“继续安装”，ADB 返回 `Success`；`pm path` 可读，冷启动 `.MainActivity` 后首页正常可交互并保留在手机上。
- 本轮截图：`/private/tmp/navbar-phone-search.png`、`/private/tmp/navbar-phone-theme-toggle.png`、`/private/tmp/navbar-phone-close.png`、`/private/tmp/navbar-phone-more.png`。
