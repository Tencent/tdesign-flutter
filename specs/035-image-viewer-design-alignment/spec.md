...
- 关闭按钮、单击、系统返回和外部路由关闭使用统一的 100ms 原位轻微缩小淡出；下拉超过阈值时，预览层从当前位移继续向屏幕底部滑出，与缩小淡出同步完成。
- 双击在 1 倍与内置放大倍数之间平滑切换；双指缩放限制在 1～3 倍。
- 放大时允许平移，同时暂停手势切图和自动轮播；还原到 1 倍后恢复。
- 当前页放大后，横向平移由当前图片消费，不切换到相邻图片。
- 图片解码或加载失败时显示由 TDesign 图标与颜色 token 构成的默认错误占位，不扩展重复的图片配置 API。
