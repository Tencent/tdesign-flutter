...
| `flutter test test/components/button --coverage` | 通过 | 112 个 Button 测试，含 Golden；变更生产文件 377/390 = 96.67% |
| `flutter test test/components/fab` | 通过 | 63 个 Fab 测试，拖拽边界回归通过 |
| Dialog/Dropdown/Empty/Fab 直接消费者测试 | 通过 | 确认 Button 尺寸改动未泄漏到其他组合组件 |
| `flutter analyze` + Button/Fab tests | 通过 | Flutter 3.47.0；Golden 仍以 3.32.0 作为统一基线 |
