...
2. **动画时长来源不一致**：小程序 `duration` 属性默认值为 240ms，而 Popup 样式和蒙层存在 300ms 回退值。组件运行时始终把公开属性传给 Popup 样式和蒙层，因此公开属性的 240ms 默认值是有效契约，300ms 只在参数缺失时兜底。

## 目标

- left/right 默认无圆角（对齐官方全高矩形），但保留通过 `TPopupThemeData.panelRadius` / `TPopupOptions.radius` 自定义圆角的能力（tokenTheme 可自定义）。
- 默认动画时长保持 240ms，与小程序公开 `duration` 属性的运行时默认值一致。
...
- `TPopupOptions.animationDuration` 与 `TPopupThemeData.transitionDuration` 均未设置时使用 240ms。
- 小程序样式和蒙层中的 300ms 是未收到 `duration` 时的回退值，不作为组件公开默认值。
- 显式设置任一者时仍以显式值为准。
