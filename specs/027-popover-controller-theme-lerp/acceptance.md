...
| 命令 | 结果 | 备注 |
| --- | --- | --- |
| Flutter 3.32.0 `flutter test --no-pub --coverage test/components/popover/t_popover_test.dart` | PASS，62 tests | 覆盖两层入口、所有关闭路径（含系统返回）、Controller 替换、无 Overlay 防御与 Theme lerp |
| Flutter 3.32.0 `dart run tool/check_component_coverage.dart popover` | PASS | 生产源码 LH/LF = 615/627 = 98.09% |
...
| Flutter 3.47.0 同组件测试与完整 analyze | PASS，62 tests | 清理并重新解析依赖后执行，0 error / 0 warning |
