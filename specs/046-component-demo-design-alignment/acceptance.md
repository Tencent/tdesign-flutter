...
- [x] Figma 节点 `27065:16056` 的 `0/100` 只有刻度文字，轨道仅在内部档位绘制分隔线；组件中的单游标和双游标刻度形状跳过 Material 传入的两端刻度。
- [x] 单游标和双游标的像素测试确认两端与相邻轨道同色、内部刻度仍可见；Flutter 3.32.0 与 3.44.9 组件测试均为 26/26 通过。
