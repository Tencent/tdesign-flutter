...
## 胶囊刻度端点初次复核（后续由分段轨道实现取代）

- [x] Figma 节点 `27065:16056` 的 `0/100` 只有刻度文字，轨道仅在内部档位留分割间隙；初次修复由刻度形状跳过端点，现已被下方分段轨道实现取代。
- [x] 初次修复的单游标和双游标像素测试确认两端与相邻轨道同色、内部刻度仍可见；Flutter 3.32.0 与 3.44.9 组件测试均为 26/26 通过。
- [x] Linux amd64 Flutter 3.32.0 更新 Slider 初始态和拖动后态的 light/dark 共四张 Golden，并在同一容器中无更新参数复跑 4/4 通过；相对旧基线每张仅 220 个像素变化，均位于胶囊型水平/垂直滑块刻度区域。

## 胶囊轨道与分割间隙逐像素复核

- [x] 对已有 Figma 原图裁片逐行取样：外层底轨 #E7E7E7、内层未选轨 #DCDCDC、内层已选轨 #0052D9；已选与未选分割中心均露出外层 #E7E7E7，而非另绘白线。此前仅跳过端点的实现没有满足这一点。
- [x] 胶囊轨道现在分段绘制，内层四周保持 3px 留白，内部 2px 间隙不着色；五段分割中心对应设计稿约 x=85/153/221/289（375px 页面），两端仍无刻度线。单游标、双游标使用同一绘制函数。
- [x] 组件测试检查选中和未选中间隙像素均等于外层底轨，并覆盖高密度等分的边界；Flutter 3.32.0 测试 27/27 通过、受影响文件 analyze 零告警。Linux amd64 Flutter 3.32.0 的四张 Slider Golden 已更新且无更新参数复跑 4/4 通过。Figma 原始节点的新导出受 Starter 额度限制，本次复核使用此前已保存的同页 Figma 原图裁片；边界抗锯齿仍有约 1px 的渲染器差异，不能声称整块位图完全一致。

## 2026-09-24 Tag 行盒与外框复核

- [x] 基于 `develop@97afb67859697d6451f119595e0e00adbdee49d5` 修复；没有新增或修改公开 API，不属于 breaking change。
- [x] 四档标签在 dark、light、outline、lightOutline 下统一为 40、28、24、20px；填充变体使用 1px 透明边框槽位，outline 不再比填充矮 2px。
- [x] 单行文字由 `TText` 应用字体 Token 行高、`TextLeadingDistribution.even` 与当前 `TextScaler`。Linux 中等尺寸基础标签的中文字形着墨从 y=249–260 调整为 y=248–259，24px 外框上下可见留白均为 6px；未使用固定像素位移。
- [x] 前置／关闭图标按四档尺寸使用 16、16、14、12px，小尺寸图文间距为 2px，其余为 4px；关闭图标使用占位文字色，普通 outline 使用容器背景色。
- [x] Flutter 3.32.0 与 3.47.0 的 Tag/TSelectTag 组件测试均为 66/66，Tag Demo 功能测试均为 4/4，严格 analyze 均为 0 issues；回归清单自测 19/19 通过。
- [x] Tag 生产源码覆盖率为 188/197（95.43%）。
- [x] Linux amd64 / Flutter 3.32.0 更新 `tag_page_light/dark` 与 `tag_selected_light/dark` 四张 Golden；更新后立即无更新参数复跑 4/4 通过，最终尺寸均为 375×1318。

## 2026-09-24 Tag Demo 设计布局复核

- [x] 导航栏使用设计稿的“Tag”，页面标题保留“Tag 标签”，说明补齐句号。
- [x] “圆弧标签”同一行按设计稿依次展示浅色圆弧、描边圆弧和 Mark 描边，三个实例之间均为 16px；不添加组件内部样式补丁。
- [x] Flutter 3.32.0 与 3.47.0 的 Tag Demo 功能测试均为 5/5，受影响文件严格 analyze 均为 0 issues；示例代码生成检查通过。
- [x] Linux amd64 / Flutter 3.32.0 更新四张 Tag Golden 后，无更新参数复跑 4/4 通过，页面尺寸保持 375×1318。
- [x] 使用已归档的同页 Figma 原图按 RGB 最大通道差 `> 28` 复算：分段仅平移配准的强差面积从 develop 的 7.77% 变为 7.90%，首标题单锚点从 8.79% 变为 8.92%。Demo 的结构、文案与实例数量已经逐项对应；比例未下降说明剩余差异仍包含组件颜色、边框和跨渲染器字体栅格，不能把布局补齐表述为整页像素一致。
