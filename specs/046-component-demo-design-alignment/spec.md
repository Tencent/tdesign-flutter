...
- `TSlider.variant` 作为滑块结构形态的唯一入口：`normal` 保持标准轨道，`capsule` 由组件提供 16px 外轨、四周 3px 内缩的激活轨道、20px 游标与胶囊刻度；Demo 不通过局部 `SliderTheme` 重画胶囊形态。
- 普通离散 Slider 的刻度点半径为 3px，保证激活段上的中间刻度仍可辨识，并同时作用于水平、垂直与区间形态。
- TabBar 胶囊容器使用 6px 语义圆角；纯文本徽标与图标类标签共用内容右上角锚点，不在 Demo 中逐项偏移。
