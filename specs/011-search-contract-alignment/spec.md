...
- TDesign 小程序：本地 `develop@fc42eb47b`
...
- 不增加 `maxCharacter` 公共 API；加权字符限制通过 `TextInputFormatter` 组合。
...
- 清除图标仅在聚焦、enabled、非 readOnly、clearable 且文本非空时出现。
- 组件实例 `variant/textAlignment` 优先于 `TSearchBarThemeData`。
- Search 结果列表、焦点后显示取消、加权字符限制均由 Demo 使用原生组合完成。
