...
- [x] 定位默认全宽为组件问题，在组件内统一加入安全区内侧左右 16px 外边距。
- [x] 定位“带关闭通知”多出的“按钮”为 Demo 问题并删除，保留独立的“带按钮通知”示例。
- [x] 删除声明式 Demo 的 `Transform.translate(-16px)` 旧补偿，确认 Demo 不再覆盖组件布局责任。
- [x] 关闭通知明亮展开态修复前后差异为 3,366px（1.11%）；差异包含左右边界内缩及多余按钮移除。
