...
修正已有默认样式值，并让现有 `TDialogAction.variant` 真正表达 Figma 的文字操作区：文字操作保留官方 32dp 顶间距、贴边 56dp Footer 和分隔线，基础操作保留内边距；三个及以上操作按主操作优先展示。居中 Popup 增加默认关闭的 `shrinkWrap`，仅由 Dialog 启用，以恢复 311dp 宽度和内容自适应高度。Example 层用现有 `Widget` 槽位、`TInput` 和操作区组合能力表达官方场景。
...
| 配套 | `popup/t_popup_options.dart`、`_popup_shell.dart` | 可选 center 内容自适应；默认 Popup 尺寸不变 |
...
- `TPopupOptions.center` 增加默认值为 `false` 的 `shrinkWrap`；不改变既有调用行为。
- `TDialog` 公开 API 无变化。
