...
- 设计证据：读取 Figma 页面 `24386:5278`，并逐项读取长内容 `27360:22420`、确认类 `27360:22456` / `27360:22476`、输入类 `27360:22499` / `27360:22500` 与六种图片实例。确认按钮配色、文字 Footer、图片全宽和 24dp 间距均以具体实例为准；Figma Starter 额度在继续读取关闭按钮实例时耗尽，因此关闭图标采用用户明确给出的 top/end 8dp 验收值。
- **组件修复**：`TDialog` 为真实溢出内容显示 4dp 滚动条，标题保持固定；24dp 关闭图标可见边界距 top/end 8dp，同时保留 48dp 点击热区。Demo 没有用外层样式遮盖这两项默认组件问题。
...
- GitHub PR [#1124](https://github.com/Tencent/tdesign-flutter/pull/1124) 已创建，head `33b87889583d26673f1d046a97dffaa6105c770a`；Issue #1027 的 Dialog 条目已追加该 PR。首次读取时 Spell Check、CNB 同步与 CLA 通过，analyze、test、Golden、构建、预览和代码扫描仍在运行，merge state 为 `BLOCKED`，因此不标记为远端 CI 全绿。
