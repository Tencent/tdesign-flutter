...
- 本组件原 PR：#1061，固定 head `4be2e8e1203a5bb6a1ca1e66ead24329c674dc8a`。本记录取代此前 PR 的验收结果，不表示远端 PR 已更新。
...
- 本组件生产源码覆盖率 `661/681 = 97.06%`，门禁为 95%；覆盖率由独立组件测试产生，没有混入 Demo 页面。
...
dart tool/check_component_coverage.dart date_time_picker
# example 目录；纯 Golden 仅在 Flutter 3.32.0 Linux 执行
flutter test --no-pub test/date_time_picker_demo_test.dart
flutter test --no-pub test/date_time_picker_demo_golden_test.dart
```
