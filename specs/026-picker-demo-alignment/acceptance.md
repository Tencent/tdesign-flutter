...
- 本组件原 PR：#1062，固定 head `e06b247d02febbf51d85e21d1f62b480d66b785c`。本记录取代此前 PR 的验收结果，不表示远端 PR 已更新。
...
- 两版本三个组件及调度器合计 223 项测试通过；三个 Demo 合计 16 项通过，本组件 Demo 为 5 项。最终局部修订按受影响套件复跑。
- 本组件生产源码覆盖率 `333/340 = 97.94%`，门禁为 95%；覆盖率由独立组件测试产生，没有混入 Demo 页面。
- Flutter 3.32.0 Linux：本组件 12 项 Golden；三组件合计 52 项，含 6 张完整页面、44 张通过真实点击打开的面板、2 张 Calendar 状态矩阵。全部更新后以无更新参数复跑通过。
...
dart tool/check_component_coverage.dart picker
# example 目录；纯 Golden 仅在 Flutter 3.32.0 Linux 执行
flutter test --no-pub test/picker_demo_test.dart
flutter test --no-pub test/picker_demo_golden_test.dart
```
