...
| `flutter test test/components/loading/t_loading_test.dart`（3.32.0） | ✅ 38/38 通过 | |
| `flutter test test/components/loading/t_loading_test.dart`（3.47.0） | ✅ 38/38 通过 | |
| `flutter test --coverage ...` | ✅ 99.62% | LF=266, LH=265（基线 86.15%） |
...
| 文件 | LF/LH | 覆盖率 |
| --- | --- | --- |
| t_loading.dart | 69/67 | 97.1%（未覆盖 line 92：`colorScheme?.primary` fallback 边界） |
| t_loading_controller.dart | 25/25 | 100% |
...
- **真机像素表现未实测**：本环境无真机/DPR/主题截图，尺寸/间距/蒙层对齐基于官方 CSS 数值与 Flutter 常量静态换算，需真机复核。
