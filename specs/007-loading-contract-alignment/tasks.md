...
- [x] `t_loading_controller.dart`：`show` 新增 `TOverlayConfig? overlay`（复用 toast 蒙层）
- [x] `t_loading_theme_data.dart`：`duration` dartdoc 默认说明
- [x] 示例页补 custom 指示器 Demo（含生成代码）
- [x] 补充 Widget 测试（duration/axis/overlay/尺寸/覆盖率补充）
...
- [x] `flutter test test/components/loading/t_loading_test.dart` 38 用例通过
