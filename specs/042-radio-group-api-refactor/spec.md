...
11. `.options` 的横向 `inline` 布局按内容自然收缩并换行，不使用 `columns`
    等分父约束；同一行首尾选项贴齐可用区域两端，中间余量均分，选项间至少保留
    `spacer16`。其他需要网格对齐的布局继续由 `columns` 约束。
12. `block` 项间分割线的实际线条从正文起点绘制，左侧缩进区域由容器背景承接，
    不得透出页面底色形成视觉通栏。
13. 横向 `inline` Demo 的容器使用页面完整宽度，文案区域距离容器四周均为
    `spacer16`，不得由组件和 Demo 重复叠加外围留白。
14. 状态 Demo 的两个禁用选项分别使用“单选-已选”和“单选-未选”，使选中禁用与
    未选禁用状态可以从文案直接区分。
15. 未选中且禁用的内置圆形指示器使用 `componentBorderColor` 描边和
    `bgColorComponentDisabled` 填充；浅色默认值分别为 `#DCDCDC` 与 `#EEEEEE`。
16. 纵向与横向 `card` 的文案区域均在边框内居中，边框到单行文案四周均为
    `spacer16`，选中角标不得遮挡文案。
