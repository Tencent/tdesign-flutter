...
- 默认加载和失败占位分别使用 TDesign `ellipsis`、`close` 图标；`loadingBuilder`/`errorBuilder` 优先于对应静态 Widget。
...
- 小程序 `load`/`error` 事件在 Flutter 中由 `frameBuilder`/`errorBuilder` 承接渲染生命周期；不增加会与 builder 重复触发的第二套完成源。
