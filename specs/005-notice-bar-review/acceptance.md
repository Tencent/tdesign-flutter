...
| Flutter 3.32.0 `flutter test --no-pub test/components/notice_bar --coverage` | PASS，42 tests；NoticeBar `LH=240` / `LF=250` = 96.00% |
...
| Flutter 3.32.0 `flutter test --no-pub test/notice_bar_page_golden_test.dart test/notice_bar_page_test.dart` | 功能测试 PASS；macOS Golden 高度与现有 Linux 基线相差 2px，未写回基线 |
| CNB 同款 `docker/flutter-3.32.0` Linux 镜像更新并复跑 `notice_bar_page_golden_test.dart` | 未执行；本地容器运行需要向容器暴露工作区源码，等待维护者明确授权 |
...
- [ ] 收敛后 375dp Flutter light/dark Linux Golden 尚待更新并复验；既有截图只能证明收敛前页面。
...
- 收敛后的 Flutter 3.32.0 Linux light/dark Golden 尚未生成；macOS 结果不能写回 Linux 基线。
- 逐帧叠图与真实设备触摸循环仍未验证。
