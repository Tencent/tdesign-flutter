...
- 辅助文本使用 `TText` 与完整 `fontBodyLarge` 字体 token；默认 24px 行盒与 24px 星标几何居中，额外行高在上下两侧均匀分配，`TRateThemeData.textStyle` 作为局部覆盖层合并而不丢失未覆盖的字体字段。
...
- 0 分“未评分”辅助文案使用 `textDisabledColor`，显式 Material `ColorScheme.onSurface` 保留同等透明度；局部 `TRateThemeData.textStyle` 仍可覆盖。
- “评分大小”和“评分风格”各使用两个独立容器展示，容器间距使用 `spacer16`；线描评分使用 `warningColor5`。
- 竖向带描述评分使用 `bgColorContainer` 填充容器，默认 4 分文案为“服务很棒”。
...
11. 默认辅助文本使用完整 `fontBodyLarge` 字号、行高与字重，并与评分星标按行盒中心对齐；Theme 仅覆盖显式提供的文字样式字段。
12. 启用状态下 0 分辅助文案使用禁用文字色 Token；非 0 分继续使用主文字色，禁用组件继续遵循禁用态颜色解析。
...
- 组件测试覆盖文案完整字体 token、星标与文字行盒中心对齐、默认评分提示及关闭路径、半星浮层、自定义图标、读屏增减、RTL、慢速拖拽生命周期、指针取消与受限宽度长文案。
