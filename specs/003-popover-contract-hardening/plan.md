...
- TPopover.showPopover 不再把文本模式的 theme.minWidth/theme.maxHeight 预先转成固定 width/height；后续任意 Widget 动态测量由 `specs/022-popover-public-api-convergence` 接续实现。
...
| 范围 | 文件或模块 | 影响 |
| --- | --- | --- |
| 组件入口 | tdesign-component/lib/src/components/popover/t_popover.dart | 解析内容与主题尺寸；后续公开 API 收敛见 Spec 022 |
...
| 文档 | 本 Spec、生成 API 文档 | 本阶段签名未变化；后续公开签名和生成文档由 Spec 022 更新 |

## API 变化

- 本 Spec 原始实现阶段不新增、不删除公共参数或类型；后续 breaking 变化见 Spec 022。
...
- 本 Spec 原始阶段未引入动态测量；Spec 022 后续以首帧隐藏和真实子布局尺寸完成收敛。
