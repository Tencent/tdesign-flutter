...
- iPhone 16 / iOS 18.2 模拟器实际像素复核：数字墨迹中心与控件中心仅差 1 个物理像素；Flutter 3.32.0 与 3.47.0 各 39 项组件测试通过，定向 analyze 均为 0 error / 0 warning。
- Linux amd64 / Flutter 3.32.0 CI 镜像更新亮暗两张组件 Golden 后精确复跑，2 项通过、像素差为 0；显式 `DefaultTextStyle` 与组件 Theme 的字体覆盖能力保持。
- Theme 回归复核发现，先按默认 12px 字号写入 20px 行盒倍数、再仅覆盖字号为 20px 时，会得到约 33px 的错误行盒；仅覆盖控件高度为 16px 时，默认 20px 行盒也会超出可用高度。修复后先解析最终字号和控件高度，再计算默认行盒倍数；20px 自定义字号保持 20px 行盒，16px 自定义控件高度下行盒收敛为 16px，二者组合仍不超过控件，显式行高覆盖保持不变。
