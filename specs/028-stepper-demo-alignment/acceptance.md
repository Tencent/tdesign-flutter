...
- Figma 组件源节点确认小、中、大号数字规格分别为 10/16、12/20、16/24，文字行盒在控件内垂直居中；Form 的 `年限` 项没有额外位移。
- Android 手机修复前截图中，中号数字墨迹中心比控件中心高约 5 个物理像素；组件改为完整设计行盒，并默认使用仓库内置 `TCloudNumber`，避免系统数字字体在 Android/iOS 上使用不同度量。
- iPhone 16 / iOS 18.2 模拟器实际像素复核：数字墨迹中心与控件中心仅差 1 个物理像素；Flutter 3.32.0 与 3.47.0 各 46 项组件测试通过，定向 analyze 均为 0 error / 0 warning。
- Linux amd64 / Flutter 3.32.0 CI 镜像更新亮暗两张组件 Golden 后精确复跑，2 项通过、像素差为 0；显式 `DefaultTextStyle` 与组件 Theme 的字体覆盖能力保持。
- Theme 回归复核发现，先按默认 12px 字号写入 20px 行盒倍数、再仅覆盖字号为 20px 时，会得到约 33px 的错误行盒；仅覆盖控件高度为 16px 时，默认 20px 行盒也会超出可用高度。修复后先解析最终字号和控件高度，再计算默认行盒倍数；20px 自定义字号保持 20px 行盒，16px 自定义控件高度下行盒收敛为 16px，显式行高覆盖保持不变。20px 字号与 16px 控件高度、12px 字号与 2 倍显式行高及 20px 控件高度等冲突组合会在调试模式明确断言，不再把无法容纳的配置视为有效布局。
- CNB Review 发现 Flutter `TextStyle.merge` 会在高优先级样式未设置 package 时保留底层私有 package，导致组件 Theme 的无 package 字体可能被错误解析到 TDesign 或外层字体包。修复后先把各级字体转换为 Flutter 已解析的完整字体族，再参与 Theme 合并；新增无 package 字体、package 字体、覆盖外层 package 字体及仅覆盖字号保留外层 package 字体四条组件回归。四个生产文件覆盖率为 `379/385 = 98.44%`。
