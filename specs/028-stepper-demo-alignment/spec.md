...
- 数字输入区按小、中、大尺寸分别使用 10/16、12/20、16/24 字号与行盒，行盒在控件内垂直居中；未显式配置字体时使用 TDesign 数字字体，调用方的 `DefaultTextStyle` 与组件 Theme 仍可覆盖。组件 Theme 仅覆盖字号时，默认物理行盒按最终字号重新计算倍数，并在自定义控件高度更小时收敛到可用高度；显式 `TextStyle.height` 仍具有最高优先级。最终字号大于控件高度，或显式字号与行高形成的物理行盒大于控件高度时属于无效配置，调试模式必须给出包含最终尺寸的断言，避免静默裁字或光标溢出。
