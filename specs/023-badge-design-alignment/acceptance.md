...
- Flutter 3.32.0 下 Badge 聚焦测试 38/38 通过，受影响源码及测试严格 analyze 为 0 issues。
- 修复版 debug APK 已重新构建并安装到 Android 16 真机，Dot、Number、Circle、Square 的右上角位置已向上移动 4 逻辑像素；自定义徽标改用设计稿的 48px 方形按钮，并校正为徽标中线与按钮顶边对齐。
- 自定义徽标几何回归按 Figma 节点 `27741:30841` 验证：按钮为 48×48，徽标容器左边位于按钮右边 -16px，徽标中线与按钮顶边同 Y；520dpi 真机中分别对应 156×156、52px 与 0px 纵向差。
