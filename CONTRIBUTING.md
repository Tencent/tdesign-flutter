...
3. **「不写更新日志」≠「不需要 Spec」**：是否写更新日志与是否需要创建 Spec 是**两件独立的事**，不能互相推导（详见 [`specs/README.md`](./specs/README.md)）。区分三种情况：
   - **行为不变的纯内部重构**：属于重构，**需要 Spec**（Review 结合实际改动判定），但用户感知不到，**不写更新日志**——不要因"行为不变、不写日志"就跳过 Spec。
   - **不影响公共契约的简单改动 / 单文件局部修复**：**不需要 Spec**，也不写更新日志。
   - **改变用户可感知行为的重构 / API 变更**：**既要 Spec**，**也要写更新日志**；若是 breaking change 还用 `breaking` commit type。
